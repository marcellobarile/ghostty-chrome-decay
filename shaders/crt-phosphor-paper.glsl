// Paper-white phosphor CRT shader for Ghostty (P4 white, Mac SE "paper white")
...
#define PHOSPHOR_TINT    vec3(0.95, 0.96, 1.00) // P4 white, faint cool tint
#define BRIGHTNESS       2
// ─────────────────────────────────────────────────────
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Convert luminance to white phosphor tint
    ...
    // Vertical phosphor mask (RGB stripe simulation, neutral for white)
    float vstripe = mod(fragCoord.x, 3.0);
    if (vstripe < 1.0)      color *= vec3(0.96, 0.96, 0.98);
    else if (vstripe < 2.0) color *= vec3(0.94, 0.95, 0.97);
    else                    color *= vec3(0.93, 0.94, 0.96);
