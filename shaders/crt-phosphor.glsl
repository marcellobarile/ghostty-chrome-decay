// Green phosphor CRT shader for Ghostty
...
#define PHOSPHOR_TINT    vec3(0.20, 1.00, 0.30) // P1 green
#define BRIGHTNESS       2
// ─────────────────────────────────────────────────────
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Convert luminance to green phosphor tint
    ...
    // Vertical phosphor mask (RGB stripe simulation, subtle)
    float vstripe = mod(fragCoord.x, 3.0);
    if (vstripe < 1.0)      color *= vec3(0.95, 1.00, 0.95);
    else if (vstripe < 2.0) color *= vec3(0.92, 1.00, 0.92);
    else                    color *= vec3(0.90, 1.00, 0.90);
